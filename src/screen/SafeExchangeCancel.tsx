...
export const SafeExchangeCancel = ({ navigation }: Props) =>{
  ...
  useEffect(()=>{
    const asyncRun = async() =>{
      ...
      if (keyPair!=null)
        setItems(await getExchangeCancelList(cluster,keyPair));
      setIsLoading(false);
    }
    asyncRun();
  },[])
  ...
  const exchange_cancel = async (exdata:IExdata) =>{
    setIsLoading(true);
    const initializer = await wallet.getKeyPair(account);
    if (initializer!=null) {
      await ExchangeCancel(
        cluster,
        initializer,
        exdata,
      )
      ...
      setShowIdx(-1);
      setItems(await getExchangeCancelList(cluster,initializer));
    }
    setIsLoading(false);
  }
  ...
  return(
    <>
    <Header goBack={() => navigation.navigate('SafeExchange')} title={'Cancel Exchange'}/>
            ...
            {items.map((item,index)=>{
              ...
              return (
                ...
                <TouchableOpacity className="items-center mb-2"
                  ...
                  <View className="flex-row w-4/6 h-8 mt-1 justify-between items-center bg-indigo-400 dark:bg-slate-700 rounded-full px-3">
