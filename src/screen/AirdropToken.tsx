...
import { getKeyPair, ITokenInfo } from "../wallet";
...
export const AirdropToken = ({ navigation }: Props) =>{
  const {tokenMap,cluster,update,setUpdate,account} = useAccounts();
  ...
  const handleAirdropToken = async() => {
    setIsLoading(true);
    const Keypair = await getKeyPair(account);
    if (Keypair==null) return;
    try{
      await utils.airdropSplTokens(
        cluster,
        Keypair,
        toAccount,
        Number(text),
  ...
  const info:ITokenInfo|undefined = tokenMap.get('Gh9ZwEmdLJ8DscKNTkTqPbNwLNNBjuSzaG9Vp2KGtKJr');
  const maxValue = 10000;
  return(
    <>
    <Header goBack={() => navigation.navigate('DashboardScreen')} title={'AirdropToken'}/>
    <View className={`w-full h-full  items-center  bg-indigo-300 dark:bg-slate-800`}>
      ...
      >
        ...
        {text}</Text>
        <Text className="w-30 text-slate-900 dark:text-slate-300 ml-2 text-lg text-bold mr-2">
          {info?.symbol}</Text>
      </TouchableOpacity>
      {(Number(text)>maxValue) && <View className="w-56 justify-center items-center rounded-full bg-red-100">
        <Text className='text-red-600 text-xs'>Max: 10000 USDC-Dev</Text>
      </View>}
