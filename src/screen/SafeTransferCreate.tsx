...
import {TransferInit} from "../safe_transfer/index";
...
export const SafeTransferCreate = ({navigation }: Props) =>{
    ...
    useEffect(()=>{
      setMint(account.tokens.master.mintKey); 
    ...
    const transfer_init = async () =>{
      setIsLoading(true);
      const sender = await wallet.getKeyPair(account);
      if (sender!=null) {
        await TransferInit(
          cluster,
          sender,
  ...
  return(
    <>
    <Header goBack={() => navigation.navigate('SafeTransfer')} title={'CreateTransfer'}/>
    <View className={`w-full h-full  items-center  bg-indigo-300 dark:bg-slate-800`}>
        ...
        <Portal>
          <View className="items-center w-full h-full justify-center  bg-indigo-300 dark:bg-slate-700">
            <View className="items-center justify-center w-11/12 bg-indigo-400 dark:bg-slate-700 rounded-2xl">
              <TouchableOpacity 
                onPress={()=>{setSelect(false);setMint(account.tokens.master.mintKey);setBalance(account.tokens.master)}}
              ...
              { tokens.map((item,index)=>{
                  ...
                  return (
                    ...
                    onPress={()=>{setSelect(false);setMint(item.mintKey);setBalance(item)}}
      ...
      <View
          className='flex-row w-5/6 h-14 justify-between items-center bg-indigo-400 dark:bg-slate-700 rounded-full px-3 mt-6'>
  
        <TouchableOpacity onPress={()=>{setSelect(true)}}>
          <Avatar.Image className='-ml-3'
            size={56} source={{uri: mintInfo(mint)?.logo}} />
        </TouchableOpacity>
        ...
        <TouchableOpacity onPress={()=>{setSuccess('Send')}}>
          <Text className="w-30 text-slate-900 dark:text-slate-300 mr-3 text-bold ">
            {mintInfo(mint)?.symbol}</Text>
        </TouchableOpacity>
        </View>
       
        <View className="w-56 flex-row justify-center items-center">
            {(Number(amount)>maxValue) 
                ?<Text className='text-red-600 text-xs'>Max:  {Number(maxValue).toFixed(2)} {mintInfo(mint)?.symbol}</Text>
      ...
      <View className="flex-row w-5/6 h-14 justify-between items-center bg-indigo-400 dark:bg-slate-700 rounded-full px-3 mt-1">
        <View className="flex-row items-center">
          ...
          <Text style={{fontFamily: 'monospace', fontSize: 12, fontWeight:"bold"}}
            className={"text-slate-900 dark:text-slate-300"}
            >{receiver.slice(0,22)}</Text>
            <Text style={{fontFamily: 'monospace', fontSize: 12, fontWeight:"bold"}}
            className={"text-slate-900 dark:text-slate-300"}
            >{receiver.slice(22)}</Text>
          </View>
        </View> 
      ...
      {(receiver == account.tokens.master.publicKey) && 
          <View className="w-56 flex-row justify-center items-center">
            <Text className='text-red-600 text-xs'> From is the same as To </Text>
          </View>
      }
      { (chip.length > 0 && receiver.length == 0) 
          ?<TouchableOpacity
            onPress={()=>setReceiver(chip)}
        ...
        { (receiver.length > 0 && Number(amount) > 0 && 
          (Number(amount)<=maxValue) && (receiver != account.tokens.master.publicKey)) 
          ?<TouchableOpacity 
      ...
      <Portal>
        ...
        <View className=" w-full h-3/4 items-center">
          ...
          <View className="flex-row w-5/6 h-14 justify-between items-center bg-indigo-400 dark:bg-slate-700 rounded-full px-3 mt-6">
          <Avatar.Image className='-ml-3'
            size={56} source={{uri: mintInfo(mint)?.logo}} />
            <Text className="w-30 text-slate-900 dark:text-slate-300 ml-2 text-lg text-bold"
            >{amount}</Text>
