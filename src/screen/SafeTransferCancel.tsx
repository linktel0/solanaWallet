...
import {getTransferCancelList,ITransferDate,TransferCancel} from "../safe_transfer/index"
...
export const SafeTransferCancel = ({ navigation }: Props) =>{
  ...
  useEffect(()=>{
    const asyncRun = async() =>{
      ...
      if (keyPair!=null)
        //await getExchangeList(cluster,keyPair);
        setItems(await getTransferCancelList(cluster,keyPair));
      setIsLoading(false);
    }
    asyncRun();
  },[])
  ...
  const transfer_cancel = async (exdata:ITransferDate) =>{
    setIsLoading(true);
    ...
    if (initializer!=null) {
      ...
      setShowIdx(-1);
      setItems(await getTransferCancelList(cluster,initializer));
    }
    setIsLoading(false);
  }
  ...
  return(
    <>
    <Header goBack={() => navigation.navigate('SafeTransfer')} title={'Cancel Transfer'}/>
            ...
            {items.map((item,index)=>{
              ...
              return (
                ...
                <TouchableOpacity  
                  ...
                  className={`mb-2 h-10 justify-center bg-indigo-500 dark:bg-slate-600 rounded-full`}
                >
